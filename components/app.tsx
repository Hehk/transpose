...
function Query ({ query }: { query: string }) {
  return <h2 className="text-3xl mb-4"><span className="text-indigo-500">"</span>{query}<span className="text-indigo-500">"</span></h2>
}

function Container ({ children } : { children: React.ReactNode }) {
  return <main className="py-16 w-full max-w-7xl mx-auto px-2 xm:px-6 lg:px-8 h-full ">
    {children}
  </main>
}
        ...
        <Container>
          ...
          <button
            ...
            onClick={async () => {
              ...
              const children = images.map(s => ({ source: s, edit: { type: 'none' as const }, children: [] }))
      ...
      return <Container>
        ...
        <div className="grid grid-cols-3 gap-4">
          {state.children.map(node => {
            return <button className="relative h-48 border-2 border-black hover:border-indigo-500 focus:border-indigo-500 outline-none hover:shadow-md focus:shadow-md"><Image layout="fill" src={node.source} alt="image" /></button>
          })}
        </div>
        </FocusScope>
      </Container>
